...
# start_RU_port = ['Ust Luga', 'Novorossiysk', 'Primorsk']
...
#     for tup in zip_iter_and_nrport:
            ...
            start_time = time.time()
            upperbound_time = 24*21#float('inf')
            lowerbound_time = 0
            win_time_slide = 1
            strike = 'None'
            loop = True
            ...
            RU_to_NL_con = False
            port_of_interest = bwtcentr_ports
            outputpath = f'./processing/pr_inter_output/potential_routes_noloop_nrRU_{len(start_RU_port)}_time1m_nrtotport{nr_of_port}.joblib'
            # Round 1
...
#     runtime_path = f'./processing/pr_inter_output/timeandmem_loop_RU_{len(start_RU_port)}_2w_port_{zipl[0]}.joblib'
...
if __name__ == "__main__":
    for zipl in zip_iter_and_nrport:
        sys_log_file = f'./processing/pr_inter_output/sys_log_file_loop_RU_{len(start_RU_port)}_1m_port_{zipl[0]}.csv'
        proc_log_file = f'./processing/pr_inter_output/proc_log_file_loop_RU_{len(start_RU_port)}_1m_port_{zipl[0]}.csv'
...
final_route_RU_to_NL = joblib.load('./processing/pr_inter_input/loop_potential_routes_nrRU_1_nrtotport5.joblib')
route_RU_int_NL_matched_imoNr,trip_freq_dict, country_seq, port_sequence = pr.route_seq_matched_nrimo_par(
    final_route_RU_to_NL, alltankers_adjusted, 2, 2,  False, oiltype = 'all', loop_type = 'country')
aa = alltankers_adjusted.loc[test1[1]]
# %%
...
#     # for tup in zip_iter_and_nrport:
            ...
            RU_to_NL_con = True
            port_of_interest = bwtcentr_ports
            outputpath = f'./processing/pr_inter_output/wloop_potential_routes_nrRU_{len(start_RU_port)}_nrtotport{nr_of_port}.joblib'
            # Round 1
            ...
            return final_route_RU_to_NL
# time_and_men = []
# runtime_path = f'./processing/pr_inter_output/timeandmem_RU_{len(start_RU_port)}_wloop.joblib'
# import psutil, os, time, threading
...
# percent_used = (peak / total_ram) * 100

# print(f"Peak memory usage: {peak:.2f} MiB "
...
route_RU_int_NL_matched_imoNr,trip_freq_dict, country_seq, port_sequence = pr.route_seq_matched_nrimo_par(
    final_route_RU_to_NL, alltankers_adjusted, 5, 4,  False, oiltype = 'all', loop_type = 'country')
# aa = []
# for df in a:
